  ...
  run: async (client, interaction) => {
    ...
    if (!interaction.member.permissions.has('ManageMessages') && !interaction.member.roles.cache.some((r) => r.name === "Giveaways")) {
      return interaction.reply({
        content: '<:admin:1188605135211991130> You need to have the manage messages permissions to start giveaways.',
        ephemeral: true
      });
    }
    ...
    if (!giveawayChannel.isTextBased()) {
      return interaction.reply({
        content: '<:admin:1188605135211991130> Please select a text channel!',
        ephemeral: true
      });
    }
   if(isNaN(ms(giveawayDuration))) {
    return interaction.reply({
      content: '<:admin:1188605135211991130> Please select a valid duration!',
      ephemeral: true
    });
  }
    if (giveawayWinnerCount < 1) {
      return interaction.reply({
        content: '<:admin:1188605135211991130> Please select a valid winner count! greater or equal to one.',
      })
    }
    ...
    if (bonusRole) {
      if (!bonusEntries) {
        return interaction.reply({
          content: `<:admin:1188605135211991130> You must specify how many bonus entries would ${bonusRole} recieve!`,
          ephemeral: true
        });
      }
    }
    ...
    if (invite) {
      ...
      if (!client_is_in_server) {
          const gaEmbed = {
            ...
            title: "Server Check!",
            description:
              "Hey! I see a new server. Am I in it? You need to invite me before you can make that a requirement. 😳",
            timestamp: new Date(),
            footer: {
              iconURL: client.user.displayAvatarURL(),
              text: "Server Check"
    ...
    const data = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../storage/count.json')).toString());
    data.total++;
    data.normal++;
    fs.writeFileSync(path.resolve(__dirname, '../storage/count.json'), JSON.stringify(data));
