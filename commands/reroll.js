...
module.exports.run = async (client, message, args) => {
    ...
    if(!message.member.permissions.has('ManageMessages') && !message.member.roles.cache.some((r) => r.name === "Giveaways")){
        return message.reply('<:admin:1188605135211991130> You need to have the manage messages permissions to reroll giveaways.');
    }

    // If no message ID or giveaway name is specified
    if(!args[0]){
        return message.reply('<:admin:1188605135211991130> You have to specify a valid message ID!');
    }
